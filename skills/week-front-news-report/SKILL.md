...
user-invocable: true
allowed-tools: Task, WebFetch, Read, Write, Shell(mkdir*), Shell(date*), Shell(ls*), mcp__chrome-devtools__*
---

# Week Front News Report

> **整体架构**：主 Agent 调度 + SubAgent 执行 + 浏览器抓取 + 智能缓存
...
输出:
  - 目录: NewsReport/
  - 文件名: YYYY-WXX-news-report.md (ISO 周格式，如 2026-W04)
  - 格式: 标准 Markdown
...
*Generated by Week News Report v3.0*
...
3.  **用户提示**:
    -   必须在生成的周报开头（引用块部分）包含明显的警告信息，提示用户当前正在运行于降级模式。
